...
echo "[$(timestamp)] cross-platform BSR dashboard cron start"
...
"$PYTHON_BIN" .agents/workflows/run_sorftime_weekly_workflow.py
status=$?
if [ "$status" -ne 0 ]; then
  echo "[$(timestamp)] [ERROR] dashboard cron failed exit_code=$status"
else
  echo "[$(timestamp)] cross-platform BSR dashboard cron finished"
fi
exit "$status"
